...
# Add block membership
clu <- signed_blockmodel(g, k = 2, alpha = 0.5, annealing = TRUE)
g <-g |> activate(nodes) |> mutate(block=clu$membership)

# Examine neighbours of Centaurea nigra, Lotus corniculatus and Rumex acetosa
centaurea <- g |> convert(to_local_neighborhood,
                                  node = which(.N()$name == "Centaurea_nigra"),
                                  order = 1,
                                  mode = "all")

lotus <- g |> convert(to_local_neighborhood,
                                 node = which(.N()$name == "Lotus_corniculatus"),
                                 order = 1,
                                 mode = "all")

rumex <- g |> convert(to_local_neighborhood,
                                 node = which(.N()$name == "Rumex_acetosa"),
                                 order = 1,
                                 mode = "all")

ggraph(lotus, layout = "focus", focus = 6) +
  geom_edge_link0(aes(edge_colour = sign)) +
  geom_node_point(aes(fill = block, size = hits), shape = 21) +
  
  # geom_node_text(aes(filter = (name == "Ned"), size = hits, label = name),
  #                family = "serif") +
