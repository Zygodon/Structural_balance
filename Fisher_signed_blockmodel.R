...
print(balance_score(g, method = "triangles"))
...
# Find 2 blocks corresponding to near complete balance score
k = 2
clu <- signed_blockmodel(g, k = k, alpha = 0.5, annealing = TRUE)
# Block membership in g2
g2 <-g |> activate(nodes) |> mutate(block=clu$membership)
tbl <- g2 |> activate(nodes) |> as_tibble()|> select(name, block)
plot(ggblock(g2,clu$membership,show_blocks = TRUE))
print(paste("k =", k, ", criterion =", clu$criterion))
