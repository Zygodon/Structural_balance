...
balance <- balance %>%
  mutate(outlier = ifelse(findoutlier(balance), TRUE, FALSE))

g <- g |> activate(nodes) |> mutate(nbrs = local_members(mindist = 1))
g <- g |> activate(nodes) |> left_join(balance)
g1 <- g |> activate(nodes) |> filter(outlier) # All 6 outliers here

# Get neighbours of outliers
nbrs <- g1 |> activate(nodes) |> select(nbrs) |> as_tibble()
nbrs <- tibble(species_data$name[unlist(nbrs$nbrs)]) |> rename(name = 1)
nbrs <- nbrs |> left_join(species_data)

g2 <- g |> activate(nodes) |> select(-nbrs) |> filter(name %in% nbrs$name) # A graph with just nbrs species from all of g
g3 <- graph_join(g1, g2)
...
g4 <- g4 |> 
  ...
  left_join(balance) |>
  select(-threshold, -balance)

# plot using ggraph
ggraph(g4, layout = 'kk') + 
  geom_edge_fan(aes(colour = factor(sign)), show.legend = TRUE) + 
