...
# Classify each quadrat as b1 or b2 according to whether it has more b1 species or more b2 species.
block_tbl <- g |> activate(nodes) |> select(-jit) |> as_tibble()
the_data <- the_data |> rename(name = species_name)
q_blocks <- left_join(the_data, block_tbl, by = "name") |> select(quadrat_id, name, hits, freq, block) |> filter(!is.na(block))
q_block_counts <- q_blocks |> 
  group_by(quadrat_id, block) |> 
  count() |> 
  pivot_wider(names_from = block, values_from = n) |>
  rename(block_1 = "1", block_2 = "2") |>
  ungroup() |> 
  mutate(b1_max = block_1 >= block_2) |> # Could jitter to randomise ties
  mutate(max_block = ifelse(b1_max, "b1", "b2")) |>
  select(-b1_max)
