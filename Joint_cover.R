...
q <- sprintf('select quadrat_id, domin, species.species_name from surveys
      join quadrats on quadrats.survey_id = surveys_id
      join visit_dates on quadrats.vd_id = visit_dates.vds_id
      join records on records.quadrat_id = quadrats_id
      join species on species.species_id = records.species_id where species.species_id != 4 and
      major_nvc_community = "MG5" and quadrat_size = "2x2";')
rs1 = dbSendQuery(con, q)
domin_data <- as_tibble(fetch(rs1, n=-1)) |> rename(name = species_name)
dbDisconnectAll()
rm(con, q, rs1)

JointCover <- function(A, B){
  domins_A <- domin_data |> 
    filter(name == A) |>
    rename(A = name, dom_A = domin)
  domins_B <- domin_data |> 
    filter(name == B) |>
    rename(B = name, dom_B = domin)
  
  jc <- domins_A |> 
    left_join(domins_B, join_by(quadrat_id)) |> 
    filter(!is.na(B)) |> 
    rowwise() |> 
    mutate(contact = sum(dom_A, dom_B)) |> 
    ungroup()
  return(median(jc$contact))
}
